...
def fit_mean(t): # t\in [0,7], this function is designed to calculate the fitted mean of control at time t
    if (t<=0.3):
        f = 0.65-(0.35)/0.3*t
    if (0.3<t<=0.6):
...
def fit_sd(t): #t\in [0,7], this function is designed to calculate the fitted standard deviation of control at time t
    if (t<=0.3):
        f = 0.8-0.4/0.3*(t)
    if (0.3<t<=0.6):
...
def fit_mean_treat(t): #t\in [0,7], this function is designed to calculate the fitted mean of treatment at time t
    if (t<=0.5):
        tmp = 0.3+0.17/0.5*(t)
    if (0.5<t<=1):
...
def fit_sd_treat(t): #this function is designed to calculate the fitted standard deviation of treatment at time t
    if (int(t-0.5)<int(t)):
        s = t-int(t)
        f = 0.03+0.04/0.5*s
    else:
...
def fit_arrival_rate(t): #this function is designed to calculate the fitted tensity of nonhomogeneous Poisson process at time t
    if (t<=0.5):
        f = 0.3+0.4/0.5*(t)
    if (0.5<t<=1):
...
def est_true_effect(t):  #this function is designed to help calculate the true treatment effect at time t
...
prob_area = np.zeros((6,300))  #(i,j)-th element stands for q_j when k(n) is the i-th element of [1,14,21,42,84,168]
#calculate prob_area
...
def generate_arrival_process(lambda_1,T): #generate an arrival process with \bar\lambda=lambda_1 and time interval [0,T]
...
def generate_arrival_process_sec_4(n,T):  #generate an arrival process that used in section 4 with n customers
    arrival_time = []
    for i in range(n):
        tmp = np.random.random()
        arrival_time.append(tmp)
...
def generate_outcome(t,treat_label): #generate the outcome at time t. Here treat_label=1 iff the customer is assigned to treatment, otherwise it's 0
...
def changing_p(t): #In section 4, p(t) is changing through t. Here p(t) is slightly different from the paper but doesn't change the conclusion.
    tmp = int(t*2)
    p = 0.1+tmp*0.4/13
    return p


def nonstationary_abtest(lambda_bar, T):  #run all the algorithms with \bar\lambda and time interval [0,T]
    effect_K = np.zeros((6))   #treatment effect with post stratificatoin
    effect_1 = np.zeros((6))   #treatment effect with time-group randomization
    effect_Q = np.zeros((6))  #treatment effect with debiased stratification
    effect_naive = np.zeros((1)) #naive sample difference estimator

    kn = int(lambda_bar ** 0.4 * 3 / 7) * 7   #initialize k(n) and calculate q_j
    for i in range(kn):
        t_1 = (i) * 7 / kn
        t_2 = (i + 1) * 7 / kn
        area_tmp, err_tmp = integrate.quad(fit_arrival_rate, t_1, t_2)
    ...
    # experiment design: i.i.d. bernoulli data with p=0.5, generate data
    z = np.zeros((n))
    for i in range(n):
        tmp = np.random.random()
        if tmp > 0.5:
    ...
    for l in range(6):         #calculate the post stratification estimator for k(n) in [1, 7, 21, 42, 84, kn]
    ...
    #calculate the time-grouped randomization estimator with different strata
    for l in range(6):
        array_tmp = np.array([1, 7, 21, 42, 84, kn])
        K = array_tmp[l]
        num_outcome = np.zeros((K, 2))
    ...
    #calculate the debiased post stratification estimator with different strata
    for l in range(6):
...
def main_exp(ii):  #main experiments, for multiprocessing
    tmp = nonstationary_abtest(8000,7)
    return tmp
...
if __name__ == "__main__":
    ...
    file2write = open("ABtest_N_8000_T_100000.txt", 'w')
    print(mse_1, file=file2write)   #output line 1: mse of alpha_tr with different strata k(n)=[1, 14, 21, 42, 84, kn]
    print('\n', file=file2write)
    print(mse_2, file=file2write)   #line 2: mse of alpha_sps with different strata
    print('\n', file=file2write)
    print(mse_3, file=file2write)   #line 3: mse of alpha_dps with different strata
    print('\n', file=file2write)
    print(est_var, file=file2write)  #line 4: var of alpha_dps with different strata
    print('\n', file=file2write)
    print(est_biased-true_effect, file=file2write)  #line 5: bias of alpha_dps with different strata
    print('\n', file=file2write)
    print(effect_naive_ave-true_effect, file=file2write) #line 6: bias of naive estimator
    print('\n', file=file2write)
    file2write.close()
